 ...
 * @since 4.3.1
 */
public class FaceChatCallHandler extends TextWebSocketHandler {

	private static final Logger log = LoggerFactory.getLogger(FaceChatCallHandler.class);
	...
	@Autowired
	private FaceChatUserRegistry registry;

	@Override
  public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
    JsonObject jsonMessage = gson.fromJson(message.getPayload(), JsonObject.class);
    FaceChatUserSession user = registry.getBySession(session);
...
//      test======================
	...
	private void register(WebSocketSession session, JsonObject jsonMessage) throws IOException {
		...
		FaceChatUserSession caller = new FaceChatUserSession(session, name);
	...
	private void call(FaceChatUserSession caller, JsonObject jsonMessage) throws IOException {
		...
		if (registry.exists(to)) {
			...
			FaceChatUserSession callee = registry.getByName(to);
	...
	private void incomingCallResponse(final FaceChatUserSession callee, JsonObject jsonMessage) throws IOException {
		String callResponse = jsonMessage.get("callResponse").getAsString();
		String from = jsonMessage.get("from").getAsString();
		final FaceChatUserSession calleer = registry.getByName(from);
	...
	public void stop(WebSocketSession session) throws IOException {
		...
		if (pipelines.containsKey(sessionId)) {
			...
			FaceChatUserSession stopperUser = registry.getBySession(session);
			if (stopperUser != null) {
				FaceChatUserSession stoppedUser = (stopperUser.getCallingFrom() != null)
